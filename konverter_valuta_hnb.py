...
def dohvati_sve_valute():
    ...
    if response.status_code == 200:
        ...
        valute = [
            {"valuta": el["valuta"], "tecaj": el["srednji_tecaj"]} for el in podaci
        ]
        return valute
    else:
...
def dohvati_tecaj(valuta):
    ...
    tecaj = [el for el in valute if el["valuta"] == valuta]
    newTecaj = tecaj[0]["tecaj"]
    newTecaj = newTecaj.replace(",", ".")
    return float(newTecaj)
...
valute = dohvati_sve_valute()
imeValuti = [el["valuta"] for el in valute]

vasa_valuta_entry = ttk.Combobox(
    vasa_valuta_label, values=imeValuti, font=("Helvetica", 24)
)
...
unos_vlaute_za_konverziju = ttk.Combobox(
    konverzija, values=imeValuti, font=("Helvetica", 24)
)
...
clear_button = Button(okvir_konverzije, text="Clear", command=clear)
